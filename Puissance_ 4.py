...
#### Placement/Supression d'un pion pour un joueur ####
...
def undo():
    # permet de supprimer un pion (cercle)
    global HISTORIQUE
    global memoire
    if len(OBJETS) != 0:
...
#### Vérification si victoire d'un joueur ####          

def test_puissance4():
    #Cette partie du programme est inspiré d'un code trouvé sur open classrooms, elle fonctionne partiellement 
    ...
    #difficultés pour vérifier si 4 jetons sont sur la diagonale, ne fonctionne pas tout le temps
    for i in range(3):
        for j in range(4):
            diag = ""
            for h in range(4):
...
#### Affichage/suppresion des pions via interface graphique ####
...
def historique(lig,col):
    # Permet de garder en mémoire les pions déjà posés 
    global HISTORIQUE
